...
input=(
  'Gazebo' 'roslaunch mrs_simulation simulation.launch world_name:=grass_plane gui:=true
  '
  'Spawn' 'sleep 10; rosservice call /mrs_drone_spawner/spawn "$UAV_NUMBER $UAV_TYPE --enable-rangefinder --enable-ground-truth --enable-mobius-camera-front"
  '
  'Status' 'sleep 15; roslaunch mrs_uav_status status.launch
  '
  'Control' 'sleep 20; roslaunch mrs_uav_general core.launch
  '
  'AutomaticStart' 'sleep 20; roslaunch mrs_uav_general automatic_start.launch
  '
  'ArmAndOffboard' 'sleep 25; rosservice call /$UAV_NAME/mavros/cmd/arming 1; sleep 2; rosservice call /$UAV_NAME/mavros/set_mode 0 offboard
  '
  'EdgeDetector' 'sleep 30; roslaunch vision_example vision_example.launch
  '
  'GoTo' 'rosservice call /$UAV_NAME/control_manager/goto "goal: [0.0, 10.0, 1.5, 0.0]"'
  'Waypoint' 'roslaunch waypoint_flier waypoint_flier.launch'
  'StartWaypoint' 'rosservice call /$UAV_NAME/waypoint_flier/fly_to_first_waypoint'
  'StartFollowing' 'rosservice call /$UAV_NAME/waypoint_flier/start_waypoints_following'
  'StopFollowing' 'rosservice call /$UAV_NAME/waypoint_flier/stop_waypoints_following'
  'rviz1' 'sleep 15; roslaunch mrs_uav_testing rviz.launch
  '
  'rviz2' 'sleep 16; roslaunch mrs_rviz_plugins load_robot.launch
  '
  'gzcamera_follow' 'sleep 20;waitForOdometry; gz camera -c gzclient_camera -f $UAV_NAME
  '
)
